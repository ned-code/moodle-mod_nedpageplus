...
function nedpageplus_add_instance($data, $mform = null) {
    ...
    $data->id = $DB->insert_record('nedpageplus', $data);
    ...
    \core_completion\api::update_completion_date_event($cmid, 'nedpageplus', $data->id, $completiontimeexpected);
...
function nedpageplus_delete_instance($id) {
    ...
    $cm = get_coursemodule_from_instance('nedpageplus', $id);
    \core_completion\api::update_completion_date_event($cm->id, 'nedpageplus', $id, null);
...
function nedpageplus_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    ...
    if ($filearea === 'content') {
        ...
        return new nedpageplus_content_file_info($browser, $context, $storedfile, $urlbase, $areas[$filearea], true, true, true, false);
    }

    // note: nedpageplus_intro handled in file_browser automatically
...
function nedpageplus_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if ($arg == 'index.html' || $arg == 'index.htm') {
        ...
        if (!$file = $fs->get_file_by_hash(sha1($fullpath)) or $file->is_directory()) {
            ...
            $DB->update_record('nedpageplus', $page);
        }
...
function nedpageplus_dndupload_handle($uploadinfo) {
    ...
    return nedpageplus_add_instance($data, null);
}
...
function nedpageplus_view($page, $course, $cm, $context) {
    ...
    $event->add_record_snapshot('nedpageplus', $page);
    $event->trigger();
...
function mod_nedpageplus_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \moodle_url('/mod/nedpageplus/view.php', ['id' => $cm->id]),
        1,
        true
    );
}