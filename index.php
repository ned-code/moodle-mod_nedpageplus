...
$event = \mod_nedpageplus\event\course_module_instance_list_viewed::create(array('context' => context_course::instance($course->id)));
$event->add_record_snapshot('course', $course);
$event->trigger();

$strpage         = get_string('modulename', 'nedpageplus');
$strpages        = get_string('modulenameplural', 'nedpageplus');
$strname         = get_string('name');
$strintro        = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_url('/mod/nedpageplus/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.': '.$strpages);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strpages);
echo $OUTPUT->header();
echo $OUTPUT->heading($strpages);
if (!$pages = get_all_instances_in_course('nedpageplus', $course)) {
...
foreach ($pages as $page) {
    ...
    $table->data[] = array (
        ...
        format_module_intro('nedpageplus', $page, $cm->id));
}
